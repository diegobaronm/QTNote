...
After calculating the integrated luminosity uncertainty for the SFs, we find that it is less than $0.07\%$, so we just ignore it in our final results. For our uncertainties calculation we use the standard ATLAS procedure. In this method, the objects are re-calibrated using parameters one sigma away from the central value. The results are then compared between the nominal and the variation events and the difference is quoted as the uncertainty. 

 The systematic uncertainties used to report the SFs are presented in Table \ref{Tab5}. These numbers have been provided by Terry Wyatt and Sam Dysch, based on their previous experience working on an analysis that makes use of $Z\to\tauhad\taulep$ events. A full estimate of the systematic uncertainties is in progress.
\begin{table}[htbp]
	\centering
	\begin{tabular}{cc}
		\hline
